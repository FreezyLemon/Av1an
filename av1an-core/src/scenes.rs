...
impl Scene {
  pub fn parse_from_zone(input: &str, encode_args: &EncodeArgs) -> Result<Self> {
    ...
    if encoder.format() != encode_args.encoding.encoder.format() {
      bail!(
        "Zone specifies using {}, but this cannot be used in the same file as {}",
        encoder,
        encode_args.encoding.encoder,
      );
    }
    if encoder != encode_args.encoding.encoder {
      if encoder
        .get_format_bit_depth(encode_args.ffmpeg.output_pixel_format.format)
        .is_err()
      {
        bail!(
          "Output pixel format {:?} is not supported by {} (used in zones file)",
          encode_args.ffmpeg.output_pixel_format.format,
          encoder
        );
      }
      if !reset {
    ...
    let mut video_params = if reset {
      ...
      encode_args.encoding.params.clone()
    };
    let mut passes = if reset {
      encoder.get_default_pass()
    } else {
      encode_args.encoding.passes
    };
    let mut photon_noise = if reset {
      None
    } else {
      encode_args.encoding.photon_noise_level
    };
    let mut extra_splits_len = encode_args.scene_detection.extra_splits_len;
    let mut min_scene_len = encode_args.scene_detection.min_len;
